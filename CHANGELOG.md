...
- verified live against the running daemon on this machine: forced state=hot via a temporary hot_avail_pct clamp override (no real memory pressure induced) and a real fork/zombie fixture (same pattern as tests/smoke.sh) to exercise every conditional row — alert banner, hero, swap, top process, zombie count, advise headline — zero JS errors in the gnome-shell journal across a disable/enable reload; config and daemon state restored afterward
